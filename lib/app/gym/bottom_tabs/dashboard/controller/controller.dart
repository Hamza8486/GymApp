...
class GymController extends GetxController {
  ...
  List<String> gymType = ['yoga', 'pilates', 'boxing','karate','mma','swimming','athletics'];
  updateTime(val) {
    time.value = val;
    update();
  }
  ...
  updateTypeName(var val) {
    if (selectTypeName.contains(val)) {
      selectTypeName.remove(val);
    } else {
      selectTypeName.add(val);
    }
    update();
  }
